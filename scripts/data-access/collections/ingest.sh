...
# Install required packages using pip --user (no root required)
echo "Installing pypgstac in pod $EOAPI_POD_RASTER in namespace $FOUND_NAMESPACE..."
if ! kubectl exec -n "$FOUND_NAMESPACE" "$EOAPI_POD_RASTER" -- bash -c 'pip3 install --user pypgstac[psycopg]'; then
    echo "Failed to install pypgstac."
    exit 1
fi
...
if ! kubectl exec -n "$FOUND_NAMESPACE" "$EOAPI_POD_RASTER" -- bash -c 'export PATH="$HOME/.local/bin:$PATH" && pypgstac load collections /tmp/collections.json --dsn "$PGADMIN_URI" --method insert_ignore'; then
    echo "Failed to load collections."
    exit 1
fi

echo "Loading items..."
if ! kubectl exec -n "$FOUND_NAMESPACE" "$EOAPI_POD_RASTER" -- bash -c 'export PATH="$HOME/.local/bin:$PATH" && pypgstac load items /tmp/items.json --dsn "$PGADMIN_URI" --method insert_ignore'; then
