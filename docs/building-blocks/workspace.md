...
1. **Review IAM Client Administration**:  
   Before configuring the Workspace Building Block (BB), ensure the IAM infrastructure is deployed and refer to the [Client Administration](../iam/client-administration.md) guide for instructions on creating and managing Keycloak clients.

2. **Create the `workspace-bb` Client**:  
   When prompted by the instructions in the Client Administration guide, use the following values:

   - **`clientId`**: `workspace-bb`
   - **`rootUrl` and `baseUrl`**: `https://workspace-api-apx.${INGRESS_HOST}`
   - **`redirectUris`**: Should include `https://workspace-api-apx.${INGRESS_HOST}/*`, `https://workspace-swagger-apx.${INGRESS_HOST}/*`, and `/*`
   - **`client_secret`**: Use the secret generated by the `configure-workspace.sh` script.
   - **`authorizationServicesEnabled`**: `true` to enable Keycloak’s built-in authorisation capabilities if you plan to protect specific resources.

3. **Record the Client Credentials**:  
   After creation, note the `clientId` and `client_secret`. These are required for integrating the Workspace BB with IAM.
...
### 9. Apply Resource Protection (Optional)

If you wish to restrict access to certain Workspace endpoints or operations, you can leverage Keycloak policies and resource protection. This allows you, for example, to limit access to specific groups or roles.

1. **Review IAM Advanced Configuration**:  
   Refer to the [Advanced Configuration](../iam/advanced-configuration.md#applying-resource-protection-with-keycloak-policies) guide for detailed instructions on creating groups, adding users, and defining policies/resources/permissions in Keycloak.

2. **Apply Policies to `workspace-bb` Client**:  
   In the advanced configuration steps, substitute references to `myclient` or `oapip-engine` with `workspace-bb` as the `clientId`. Identify the paths or endpoints you wish to protect (e.g., `/some-secure-path`) and follow the same pattern to create policies and permissions.

3. **Test and Validate**:  
   Once policies and permissions are applied, ensure that only authorised users or groups can access the protected endpoints by testing token acquisition and making requests as described in the advanced IAM documentation.
