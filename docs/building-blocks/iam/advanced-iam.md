...
Download [`resource-protection.yaml`](https://raw.githubusercontent.com/EOEPCA/deployment-guide/refs/heads/main/docs/building-blocks/iam/advanced-iam/resource-protection.yaml), replace the `<username>`/`<client-name>` placeholders with values for your setup, then apply:

```bash
source ~/.eoepca/state
kubectl apply -f resource-protection.yaml
```
