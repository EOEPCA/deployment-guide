...
1. **Run the Configuration Script**:
    ...
    First time running a script? [EOEPCA+ State](state.md) covers the shared setup questions asked before this one - the Container Registry script itself has no further prompts, and generates a `generated-values.yaml` file for the Helm deployment directly from those shared values.
