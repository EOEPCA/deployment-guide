...
!!! warning "Important"
    The NGINX ingress should **only be used for open deployments** where EOEPCA's IAM-based request authorization is **not required** - or you are integrating your own IAM approach. If you are following the IAM integration and request authorization approach described in this guide, then select the **APISIX Ingress** instead.
...
!!! note "Disclaimer"
    We recommend following the official installation instructions for the NGINX Ingress Controller. However, this quick start guide should also work for most environments.
